...
# PLOT 5
count = trainingData.groupby(by='Months since Last Donation', as_index=False)['Made Donation in March 2007'].count()
sum = trainingData.groupby(by='Months since Last Donation', as_index=False)['Made Donation in March 2007'].sum()
data = count.merge(right=sum, how='inner', on='Months since Last Donation')
data['pct_attended'] = data['Made Donation in March 2007_y'] / data['Made Donation in March 2007_x']
data.plot(kind='scatter', x='Months since Last Donation', y='pct_attended')
...
plt.show()
plt.clf()
plt.hist(trainingData['Months since Last Donation'], bins=15)
plt.show()
plt.clf()
